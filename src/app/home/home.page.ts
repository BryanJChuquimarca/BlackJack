...
export class HomePage {
  ...
  async drawCards(count: number) {
    const response = await this.http
      .get<any>(
        `https://www.deckofcardsapi.com/api/deck/${this.deckId}/draw/?count=${count}`,
      )
      .toPromise();
  ...
  async repartir() {
    if (this.apuesta_actual === 0) {
      alert(
        '⚠️ Debes hacer una apuesta antes de jugar.\n\nSelecciona una ficha para apostar.',
      );
      return;
    }
    await this.crear_mazo();
  ...
  async stand() {
    ...
    while (
      this.calculateTotal(this.crupierCards) <
        this.calculateTotal(this.playerCards) &&
      this.calculateTotal(this.crupierCards) < 21
    ) {
      //mirar si hay empates
  ...
  calculateVisibleTotal(): number {
    ...
    let visibleCards = this.crupierOcultar
      ? this.crupierCards.slice(1)
      : this.crupierCards;
